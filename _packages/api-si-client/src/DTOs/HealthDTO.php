...
final readonly class HealthDTO
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        return new self(
            ...
            databaseSiLatency:       isset($checks['database_si']['latency_ms']) ? (int) $checks['database_si']['latency_ms'] : null,
            databaseInternal:        (string) ($checks['database_internal']['status'] ?? 'unknown'),
            databaseInternalLatency: isset($checks['database_internal']['latency_ms']) ? (int) $checks['database_internal']['latency_ms'] : null,
            redis:                   (string) ($checks['redis']['status'] ?? 'unknown'),
            redisError:              isset($checks['redis']['error']) ? (string) $checks['redis']['error'] : null,
